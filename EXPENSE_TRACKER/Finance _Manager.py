...
def spend():
    os.system("python Spend.py")

def view_expense_details():
    global date, payee, desc, amnt, MoP, table
...
def edit_expense():
    ...
    def edit_existing_expense():
        global date, payee, desc, amnt, MoP, connector, table
...
dataentery_frame_bg = 'cornflowerblue'
buttons_frame_bg = 'cornsilk1'
hlb_btn_bg = 'goldenrod3'
...
root = Tk()
root.title('Finance Assistant')
root.geometry('1200x550')
root.resizable(0, 0)

Label(root, text='FINANCE ASSISTANT', font=('Noto Sans CJK TC', 15, 'bold'), bg=hlb_btn_bg).pack(side=TOP, fill=X)
...
Button(data_entry_frame, text='Add expense', command=add_another_expense, font=btn_font, width=30,
       bg=hlb_btn_bg).place(x=10, y=345)
Button(data_entry_frame, text='Expence', command=spend,font=btn_font, width=30, bg=hlb_btn_bg).place(x=10, y=400)
